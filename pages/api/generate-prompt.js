export default async function handler(req, res) {
    ...
    try {
        ...
        if (!topic || !contentRequirements) return res.status(400).json({ error: 'Missing required fields' });
        ...
        return res.status(500).json({ success: false, error: error.message || 'Internal server error', timestamp: new Date().toISOString() });
    }
}
